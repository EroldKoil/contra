...
const bossInfo = {
  stay0: { xS: 346, yS: 238, w: 54, h: 62, frames: 1, delay: 30 },
  ...
  stay1: { xS: 346, yS: 302, w: 54, h: 62, frames: 1, delay: 30 },
  ...
  stay2: { xS: 346, yS: 368, w: 54, h: 62, frames: 1, delay: 30 },
  run2: { xS: 346, yS: 368, w: 54, h: 62, frames: 2, delay: 30 },
...
export default class Boss6 extends Person {
  ...
  tryAction(camPos) {
    ...
    if (this.bullet.isActive) {
      let spr = this.bullet.sprite;
      let dx = this.bullet.vectorMove * this.bullet.moveSpeed;
      let dy = spr.y > 165 - spr.h ? 0 : this.bullet.fallSpeed;
      spr.move(contra.pjs.vector.point(dx, dy));
      spr.draw();
      const player = contra.player;
    ...
    if (this.health > 0 && this.start) {
      ...
      if (this.health < 1) {
        ...
        if (this.pose === 0) {
          ...
          if ((this.vectorMove > 0 && this.door.doorPlatform.sprite.isStaticIntersect(spr.getStaticBoxD(0, 0, this.moveSpeed))) ||
            (this.vectorMove < 0 && this.level.leftBorder.sprite.isStaticIntersect(spr.getStaticBoxA(-this.moveSpeed)))) {
            // this.pose = 1;
            this.vectorMove *= -1;
  ...
  selectState(stateName, selectByTime) {
    stateName = stateName + this.stage;
