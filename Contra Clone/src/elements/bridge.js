...
export default class Bridge extends sprObject {
  constructor(blockCount, x, y, level, game) {
    ...
    for (let i = 0; i < blockCount - 2; i += 1) {
      this.sprites.push(
        this.createSprite(level.spritesInfo.b2, level, game, this.x + 32 * (i + 1), this.y),
      );
    }
    this.sprites.push(
      this.createSprite(level.spritesInfo.b3, level, game, this.x + 32 * (blockCount - 1), this.y),
    );
